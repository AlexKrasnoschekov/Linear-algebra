...
int main()
{
    ...
    Point<long double> A(a, b);
    Point<long double> B(c, d);
    Point<long double> C(e, f);
    Point<long double> D(g, h);
    Vector<long double> v_f(A, B);
    Vector<long double> v_s(C, D);
    if (v_f.CheckIntersection(v_s)) {
        std::cout << "YES";
    } else {
        std::cout << "NO";
    }*/
